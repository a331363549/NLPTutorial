...
class Feature:
    ...
    def load_las_word_to_pos_feature(cls):
        ...
        feature_path = cls._config.get('postagger', 'las_word_t_pos_feature')
        feature_dict = cls.read_feature_from_file(feature_dict_path)
        feature = cls.read_feature_from_file(feature_path)
        return feature_dict, feature
...
class Corpus:
    ...
    def corpus_to_words_at_pos(cls, lines):
        ...
        """
        for line in lines:
            words = [u'<BOS>'] + [word.split('/')[0] for word in line.decode('utf-8').strip().split(' ')[1:] if
                                  word] + [u'<EOS>']
            pos = [u'un'] + [word.split('/')[1] for word in line.decode('utf-8').strip().split(' ')[1:] if word] + [
                u'un']
    ...
    def cal_pre_word_to_pos_feature(cls):
        ...
        counts = {vocab: dict(pos_dict) for vocab in cls._words}
        counter = Counter(cls._words)
        counter[u'UNK'] = 1
        for index in range(1, len(cls._pos)):
            counts[cls._words[index - 1]][cls._pos[index]] += 1
        feature_dict_path = cls._config.get('postagger', 'pre_word_to_pos_feature_dict')
        feature_path = cls._config.get('postagger', 'pre_word_to_feature')
    ...
    def get_batch_x(cls, words, pos, index, features):
        ...
        pwtp_x, wtp_x, lwtp_x, pptp_x, rtp_x = [0.0] * len(pwtp), [0.0] * len(wtp), [0.0] * len(lwtp), [0.0] * len(
            pptp), [0.0] * len(rtp)
    ...
    def feature_generator(cls):
        ...
        generator_list = [(i * batch_size) - 1 for i in range(int(chunk_size)) if i]
    ...
    @classmethod
    def segment_by_window(cls, word_list=None, window=3):
        """
        窗口切割
        """
        words, pos = [], []
        pre_words = cls._words if not word_list else word_list
        begin, end = 0, window
        for _ in range(1, len(pre_words)):
            if end > len(pre_words): break
            words.append(pre_words[begin:end])
            if not word_list: pos.append(cls._pos[begin:end])
